...
(
    ...
    if [ -z $JAVA_BINARY ] || [ -z $PORTAL_HOME ] || [ -z $MSK_IMPACT_DATA_HOME ] ; then
        local message="Could not run fetch-dmp-data-for-import.sh: automation-environment.sh script must be run in order to set needed environment variables (like MSK_IMPACT_DATA_HOME, ...)"
        echo $message
    ...
    if ! downloadFromS3AllStudies ; then
        local message="s3 fetch failure: DMP repository update"
        sendPreImportFailureMessageMskPipelineLogsSlack "$message"
    ...
    local in_progress_filename="$PORTAL_DATA_HOME/dmp/.S3_update_in_progress"
    if [ -f "$in_progress_filename" ] ; then
        local in_progress="$(head -n1 "$in_progress_filename")"
        if [ "true" == "$in_progress" ] ; then
