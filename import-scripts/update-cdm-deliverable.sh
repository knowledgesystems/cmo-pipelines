...
echo "`date`: Starting CDM deliverable generation and upload"
if [ ! -f $CLINICAL_SAMPLE_FILE ] || [ ! -f $SEQ_DATE_FILE ] ; then
  echo "`date`: One or more supplied files ($CLINICAL_SAMPLE_FILE and $SEQ_DATE_FILE) do not exist, exiting..."
  exit 1
fi
...
aws s3 cp $CDM_DELIVERABLE s3://cdm-deliverable/data_clinical_$STUDY_ID.txt --profile saml
if [ $? -ne 0 ] ; then
