...
""" convert_cna_to_long_format_py3.py
This script converts a data_CNA.txt file from matrix format (Hugo_Symbol rows, Sample ID columns)
to a long format with columns: SAMPLE_ID, Hugo_Symbol, Alteration.

Usage:
    python3 convert_cna_to_long_format_py3.py $INPUT_DATA_CNA_FILE_PATH [$OUTPUT_FILE_PATH]
Example:
    python3 convert_cna_to_long_format_py3.py path/to/data_cna.txt path/to/output.txt
"""
...
def convert_cna_to_long_format(data_cna_file, output_file=None):
    ...
    df_long = df_long[['SAMPLE_ID', 'Hugo_Symbol', 'Alteration']]
    
    # Determine output file path
    if output_file is None:
        # If no output file specified, create one based on input file name
        base_name = os.path.splitext(data_cna_file)[0]
        output_file = f"{base_name}_long_format.txt"
    
    # Write the long format dataframe to the output file
    df_long.to_csv(output_file, sep='\t', index=False)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Convert data_CNA.txt file from matrix format to long format (SAMPLE_ID, Hugo_Symbol, Alteration)'
    )
    parser.add_argument('data_cna_file', help='Path to input data_CNA.txt file')
    parser.add_argument('output_file', nargs='?', default=None, help='Path to output file (optional, defaults to input_file_long_format.txt)')
    ...
    convert_cna_to_long_format(data_cna_file, output_file)
