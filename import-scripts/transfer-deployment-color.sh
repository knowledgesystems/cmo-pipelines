...
function read_scalar() {
    local key="$1"
    local value
    value=$("$YQ_BINARY" -r "$key" "$BLUEGREEN_CONFIG_FILEPATH")
    if [ "$value" == "null" ] || [ -z "$value" ] ; then
        echo "Error : missing required scalar '$key' in $BLUEGREEN_CONFIG_FILEPATH" >&2
        exit 1
    fi
    printf '%s\n' "$value"
}
...
function load_bluegreen_config() {
    ...
    GIT_BINARY=$(read_scalar '.git_binary')
...
function adjust_replica_count_in_deployment_yaml_file() {
    ...
    if ! "$YQ_BINARY" --inplace ".spec.replicas = $replica_count" "$yaml_filepath" ; then
    ...
    local updated_value
    updated_value=$("$YQ_BINARY" '.spec.replicas' "$yaml_filepath" 2>/dev/null)
...
function main() {
    ...
    load_bluegreen_config "$BLUEGREEN_CONFIG_FILEPATH"
