...
REDCAP_PIPELINE_JAR_PATH = '/data/portal-cron/lib/redcap_pipeline.jar'
...
def get_observed_project_mapping(stable_id_list, temporary_directory_name):
    mapping = {}
    for stable_id in stable_id_list:
        ...
        command = 'java -jar ' + REDCAP_PIPELINE_JAR_PATH + ' --export-mode --raw-data --stable-id ' + stable_id + ' --directory ' + study_tempdir_name
...
def compare_expected_to_observed(expected_project_mapping, observed_project_mapping):
    if len(expected_project_mapping.keys()) != len(observed_project_mapping.keys()):
        print >> sys.stderr, 'error : there was a discrepancy in the number of stable_ids expected:'
...
def main():
    ...
    observed_project_mapping = get_observed_project_mapping(expected_project_mapping.keys(), temporary_directory_name)
