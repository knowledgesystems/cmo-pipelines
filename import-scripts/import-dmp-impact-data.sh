...
dlbcl_impact_notification_file=$(mktemp $MSK_DMP_TMPDIR/dlbcl-impact-portal-update-notification.$now.XXXXXX)
...
#-------------------------------------------------------------------------------------------------------------------------------------
# TEMP STUDY IMPORT: DLBCL_IMPACT
if [ $DB_VERSION_FAIL -eq 0 ] && [ -f $DLBCL_IMPACT_IMPORT_TRIGGER ] ; then
    echo "Importing dlbcl_impact_fmi_2018 study..."
    echo $(date)
    bash $PORTAL_HOME/scripts/import-temp-study.sh --study-id="dlbcl_impact_fmi_2018" --temp-study-id="temporary_dlbcl_impact_fmi_2018" --backup-study-id="yesterday_dlbcl_impact_fmi_2018" --portal-name="msk-fmi-dlbcl-portal" --study-path="$DLBCL_IMPACT_DATA_HOME" --notification-file="$dlbcl_impact_notification_file" --tmp-directory="$MSK_DMP_TMPDIR" --email-list="$email_list" --oncotree-version="${ONCOTREE_VERSION_TO_USE}" --importer-jar="$PORTAL_HOME/lib/msk-dmp-importer.jar" --transcript-overrides-source="mskcc"
    if [ $? -eq 0 ] ; then
        RESTART_AFTER_MSK_AFFILIATE_IMPORT=1
        IMPORT_FAIL_DLBCL_IMPACT=0
    fi
    rm $DLBCL_IMPACT_IMPORT_TRIGGER
else
    if [ $DB_VERSION_FAIL -gt 0 ] ; then
        echo "Not importing DLBCL_IMPACT - database version is not compatible"
    else
        echo "Not importing DLBCL_IMPACT - something went wrong with subsetting clinical studies for DLBCL_IMPACT."
    fi
fi

## TODO: Move commit to fetch-dmp-data-for-import.sh
# commit or revert changes for DLBCL_IMPACT
if [ $IMPORT_FAIL_DLBCL_IMPACT -gt 0 ] ; then
    sendFailureMessageMskPipelineLogsSlack "DLBCL_IMPACT import"
    echo "DLBCL_IMPACT subset and/or updates failed! Reverting data to last commit."
    cd $DLBCL_IMPACT_DATA_HOME ; $HG_BINARY update -C ; find . -name "*.orig" -delete
else
    sendSuccessMessageMskPipelineLogsSlack "DLBCL_IMPACT"
    echo "Committing DLBCL_IMPACT data"
    cd $DLBCL_IMPACT_DATA_HOME ; find . -name "*.orig" -delete ; $HG_BINARY add * ; $HG_BINARY commit -m "Latest DLBCL_IMPACT dataset"
fi
# END DLBCL_IMPACT import
