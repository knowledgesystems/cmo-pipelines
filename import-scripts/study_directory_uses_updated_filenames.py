...
def CANCER_STUDY_TAG():
    return "<CANCER_STUDY>"

def datatype_sheet_download_tab_separated():
    """
    This hardcoded data is an exact copy of the contents of the current datatypes sheet from the portal_importer_configuration spreadsheet.
    When updates are mde to the datatypes sheet, this can be updated by using the "File - Download - Tab Separated Values" option in the google sheets interface.
    - make sure to copy the exact contents of that download (maintaining the literal TAB characters) into the lines below, between the indented return statement line and the final triple-quote line
    """
    return """
...
def filenames_in_use_before_update(cancer_study_identifier):
    """
    This is a hardcoded list of old filenames which were recognized before the file renaming effort of mid-2021. The merge.py and subset-dmp-data.sh scripts were working with studies limited to these filenames.
    The pattern <CANCER_STUDY> is replaced with the cancer_study_identifier, as this old pattern was recognized by the importer for seg files.
    """
    FILENAMES_IN_USE_BEFORE_UPDATE_LIST = [
        "<CANCER_STUDY>_data_cna_hg18.seg",
        "<CANCER_STUDY>_data_cna_hg19.seg",
        "<CANCER_STUDY>_meta_cna_hg18_seg.txt",
        "<CANCER_STUDY>_meta_cna_hg19_seg.txt",
        "data_CNA.txt",
        "data_CNA_RAE.txt",
    ...
    for filename in FILENAMES_IN_USE_BEFORE_UPDATE_LIST:
        if "*" in filename:
            wildcard_patterns_in_use_before_update_set.add(filename.replace(CANCER_STUDY_TAG(), cancer_study_identifier))
        else:
            filenames_in_use_before_update_set.add(filename.replace(CANCER_STUDY_TAG(), cancer_study_identifier))
    return filenames_in_use_before_update_set, wildcard_patterns_in_use_before_update_set

def parse_current_datatypes(cancer_study_identifier):
    """
    the contents of the datatype sheet (in tab separated format) are parsed and all filenames (both meta and staging) are mapped to the associated datatype. The map is returned.
    """
    ...
    for line in datatype_sheet_lines:
        ...
        staging_filename = fields[5].replace(CANCER_STUDY_TAG(), cancer_study_identifier).strip()
        meta_filename = fields[9].replace(CANCER_STUDY_TAG(), cancer_study_identifier).strip()
...
def determine_cancer_study_identifier_from_meta_file(filepath):
    """
    search file at <filepath> for a cancer_study_identifier and return it (or empty string if not found)
    """
    CANCER_STUDY_IDENTIFIER_PREFIX = "cancer_study_identifier:"
    file = open(filepath, 'r')
    for line in file.readlines():
        sys.stderr.write("  -> " + line);
        line_without_whitespace = "".join(line.split()).strip()
        if line.startswith(CANCER_STUDY_IDENTIFIER_PREFIX):
            return line[len(CANCER_STUDY_IDENTIFIER_PREFIX):].strip()
    return ""

def determine_cancer_study_identifier(basedir, filenames):
    """
    search <filenames> in <basedir> for a cancer_study_identifier in any meta filename and returns it (or empty string if not found)
    """
    cancer_study_identifier = ""
    for filename in filenames:
        sys.stderr.write("looking for csi in filename '" + filename + "'\n")
        if filename.startswith("meta_"):
            filepath = os.path.join(basedir, filename)
            if os.path.isfile(filepath):
                cancer_study_identifier = determine_cancer_study_identifier_from_meta_file(filepath)
        if len(cancer_study_identifier) > 0:
            # first success is adequate
            break
    if len(cancer_study_identifier) == 0:
        sys.stderr.write("Error : unable to determine cancer study identifier from meta files in study directory : '" + basedir + "'\n")
        sys.exit(1)
...
def split_wildcard_patterns(wildcard_patterns):
    ...
    for wildcard_pattern in wildcard_patterns:
        ...
        split_wildcard_pattern["prefix"] = pattern_segments[0]
        split_wildcard_pattern["suffix"] = pattern_segments[1]
...
def study_directory_uses_updated_filenames(path):
    """
    search directory at <path> - return true if any filenames are present, which:
    ...
    cancer_study_identifier = determine_cancer_study_identifier(basedir, filenames)
    original_datatype_filenames, original_wildcard_patterns = filenames_in_use_before_update(cancer_study_identifier)
    current_datatype_filenames, current_wildcard_patterns = parse_current_datatypes(cancer_study_identifier)
    ...
    for filename in filenames:
        ...
        if not filename in current_datatype_filenames and not matches_any_wildcard_pattern(filename, split_current_wildcard_patterns):
            # filename/pattern was not found in current list
            continue
    ...
    return new_filenames_present

def usage():
    """
    report usage and exit
    """
    sys.stderr.write("usage : study_directory_uses_updated_filenames.py <study_path>\n")
    sys.exit(1)
...
def main():
    ...
    if len(sys.argv) != 2:
        usage()
    if study_directory_uses_updated_filenames(sys.argv[1]):
        sys.exit(0);
    else:
        sys.exit(1);
