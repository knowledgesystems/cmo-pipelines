...
function usage {
    echo "merge-cdm-data.sh \$COHORT_ID \$CDM_DATA_DIR \$OUTPUT_DIR"
    echo -e "\t\$COHORT_ID                      one of: ['mskimpact', 'mskimpact_heme', 'mskaccess', 'mskarcher']"
    echo -e "\t\$CDM_DATA_DIR                   path to directory containing CDM data for given cohort (input)"
    echo -e "\t\$OUTPUT_DIR                     path to directory containing study to merge with. merged output will be written to this directory"
}
