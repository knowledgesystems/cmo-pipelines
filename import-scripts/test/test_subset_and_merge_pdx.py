...
class TestSubsetAndMergePDXStudies(unittest.TestCase):
    ...
    def test_load_destination_source_patient_mappings(self):    
        self.check_destination_source_mappings(self.destination_to_source_mapping) 
        self.check_destination_source_clinical_annotations_mappings(self.destination_to_source_mapping)
        self.check_destination_source_patient_mappings(self.destination_to_source_mapping)
    ...
    def test_subset_source_step(self):
        ...
        self.check_subset_source_step(self.destination_to_source_mapping)

    def test_filter_clinical_annotations_step(self):
        shutil.rmtree(self.root_directory)
        for destination, source_to_source_mappings in self.destination_to_source_mapping.items():
            expected_directory = os.path.join(self.expected_files, "subset_source_step", destination)
        ...
        self.compare_clinical_files_for_step(self.destination_to_source_mapping, "filter_clinical_annotations_step")

    def test_rename_patients_step(self):
        shutil.rmtree(self.root_directory)
        for destination, source_to_source_mappings in self.destination_to_source_mapping.items():
            expected_directory = os.path.join(self.expected_files, "filter_clinical_annotations_step", destination)
            actual_directory = os.path.join(self.root_directory, destination)
            shutil.copytree(expected_directory, actual_directory)
        convert_cmo_to_dmp_pids_in_subsetted_source_studies(self.destination_to_source_mapping, self.root_directory)
        self.compare_clinical_files_for_step(self.destination_to_source_mapping, "rename_patients_step")

    def test_merge_source_directories_step(self):
        shutil.rmtree(self.root_directory)
        for destination, source_to_source_mappings in self.destination_to_source_mapping.items():
            expected_directory = os.path.join(self.expected_files, "rename_patients_step", destination)
            actual_directory = os.path.join(self.root_directory, destination)
            shutil.copytree(expected_directory, actual_directory)
        ...
        self.check_merge_source_directories_step(self.destination_to_source_mapping)
   
        #merge_clinical_files(destination_to_source_mapping, root_directory, lib)
        #self.check_merge_clinical_files_step(destination_to_source_mapping, "merge_clinical_files_step")
    ...
    def check_merge_source_directories_step(self, destination_to_source_mapping):
        for destination in destination_to_source_mapping:
            ...
            actual_directory = os.path.join(self.root_directory, destination)
            for clinical_file in [filename for filename in os.listdir(expected_directory) if "meta" not in filename and "temp" not in filename]:
                self.assertTrue(self.sort_and_compare_files(os.path.join(actual_directory, clinical_file), os.path.join(expected_directory, clinical_file)))
