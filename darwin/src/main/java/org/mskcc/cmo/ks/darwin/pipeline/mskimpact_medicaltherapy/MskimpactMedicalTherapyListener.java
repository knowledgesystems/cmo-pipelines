...
public class MskimpactMedicalTherapyListener implements StepExecutionListener {
    ...
    @Value("${redcap.project_title_for_darwin_medical_therapy_data_import}")
    public String redcapProjectTitle;

    @Value("${darwin.medicaltherapy_clinical_filename}")
    private String medicalTherapyFilename;

    //TODO: maybe we also need clinicalTimeline?
    ...
    public ExitStatus afterStep(StepExecution stepExecution) {
        ...
        File stagingFile = new File(outputDirectory, medicalTherapyFilename);
        if (stagingFile.exists()) {
            try {
                ...
                log.error("Error: could not persist clinical file \"" + medicalTherapyFilename + "\" in directory \"" + outputDirectory + "\" to RedCap : IO error locating/reading file");
            }
        } else {
            log.error("Error: could not persist clinical file \"" + medicalTherapyFilename + "\" in directory \"" + outputDirectory + "\" to RedCap : file does not exist");
        }
