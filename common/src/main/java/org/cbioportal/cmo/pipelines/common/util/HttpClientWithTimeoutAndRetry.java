...
public class HttpClientWithTimeoutAndRetry {
    ...
    public HttpClientWithTimeoutAndRetry() {
        ...
        this.dropDeadInstant = null; // results in a default drop dead time of 4 * maximumTimeout (4 minutes) from start of exchange()
...
System.out.println("got error: " + response.getStatusCode().toString());
                    pauseForMilliseconds(lastTimeoutUsed);
                    continue;
                }
                return response;
