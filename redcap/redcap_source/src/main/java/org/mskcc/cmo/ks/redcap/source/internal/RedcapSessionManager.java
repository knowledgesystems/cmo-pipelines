...
public class RedcapSessionManager {
    ...
    private static int FILTER_TOKENS_METHOD_UNINITIALIZED = -1;
    private static int FILTER_TOKENS_BY_PROJECT_TITLE = 0;
    private static int FILTER_TOKENS_BY_STABLE_ID = 1;
    private static int FILTER_TOKENS_NO_CANCER_STUDIES = 2;
    ...
    int selectedTokenFilterMode = FILTER_TOKENS_METHOD_UNINITIALIZED;
    String selectedTokenFilterString = null;
    ...
    public Map<String, String> getClinicalTokenMapByProjectTitle(String projectTitle) {
        checkTokens(projectTitle, FILTER_TOKENS_BY_PROJECT_TITLE);
        return new HashMap<String, String>(clinicalDataTokens);
    }

    public Map<String, String> getTimelineTokenMapByProjectTitle(String projectTitle) {
        checkTokens(projectTitle, FILTER_TOKENS_BY_PROJECT_TITLE);
        return new HashMap<String, String>(clinicalTimelineTokens);
    }

    public String getMetadataTokenByProjectTitle(String projectTitle) {
        checkTokens(projectTitle, FILTER_TOKENS_BY_PROJECT_TITLE);
        return metadataToken;
    }

    public String getNamespaceTokenByProjectTitle(String projectTitle) {
        checkTokens(projectTitle, FILTER_TOKENS_BY_PROJECT_TITLE);
    ...
    public String getMetadataTokenByStableId(String stableId) {
        checkTokens(stableId, FILTER_TOKENS_BY_STABLE_ID);
        return metadataToken;
    }

    public String getNamespaceTokenByStableId(String stableId) {
        checkTokens(stableId, FILTER_TOKENS_BY_STABLE_ID);
        return namespaceToken;
    }

    public String getMetadataTokenFilterAllStudiesIfNecessary() {
        if (metadataToken != null) {
            return metadataToken;
        }
        checkTokens("", FILTER_TOKENS_NO_CANCER_STUDIES);
        return metadataToken;
    }

    public String getNamespaceTokenFilterAllStudiesIfNecessary() {
    ...
    public boolean tokensAreInitialized() {
    ...
    private void checkTokens(String tokenFilterString, int tokenFilterMode) {
        if (selectedTokenFilterMode == FILTER_TOKENS_METHOD_UNINITIALIZED) {
            selectedTokenFilterMode = tokenFilterMode;
            selectedTokenFilterString = tokenFilterString;
        }
        if (selectedTokenFilterMode != tokenFilterMode) {
            throw new RuntimeException("Error : RedCap tokenFilterMode has changed from " +
                    Integer.toString(selectedTokenFilterMode) + " to " +
                    Integer.toString(tokenFilterMode) +
                    " : once tokens have been fetched and filtered, the filtering mode cannot be changed within the same instance of RedcapSessionManager");
        }
        if (selectedTokenFilterString != tokenFilterString) {
            throw new RuntimeException("Error : RedCap tokenFilterMode has changed from " +
                    selectedTokenFilterString + " to " + tokenFilterString +
                    Integer.toString(tokenFilterMode) +
                    " : once tokens have been fetched and filtered, the filtering mode cannot be changed within the same instance of RedcapSessionManager");
        }
        if (!tokensAreInitialized()) {
            fillTokens();
        }
    }
    ...
    private void fillTokens() {
        ...
        for (RedcapToken token : responseEntity.getBody()) {
            ...
            if ((selectedTokenFilterMode == FILTER_TOKENS_BY_STABLE_ID && token.getStableId().equals(selectedTokenFilterString)) ||
                    (selectedTokenFilterMode == FILTER_TOKENS_BY_PROJECT_TITLE  && token.getStudyId().equals(selectedTokenFilterString))) {
        ...
        if (metadataToken == null) {
            throw new RuntimeException("Error : could not locate token for RedCap metadata project with stable id \"" + metadataProjectStableId + "\"");
        }
    }
